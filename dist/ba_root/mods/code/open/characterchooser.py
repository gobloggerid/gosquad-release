...
this will also let players choose server specific custom characters .
so no more sharing of character file with all players,
...
def new_init(
    self, vpos: float, sessionplayer: bs.SessionPlayer, lobby: bascenev1.Lobby
) -> None:
    ...
    for name in bs.app.classic.spaz_appearances:
        if name not in self._character_names and name not in self.hidden_chars:
            self._character_names.append(name)
    # Note: this is just our local index out of available teams; *not*
    # the team-id!
    self._selected_team_index = self.lobby.next_add_team
...
def _set_ready(self, ready: bool) -> None:
    ...
    from code.open import profanity
    ...
    if ready == 1:
        ...
        device_profiles = bs.app.config.setdefault(
        ...
        if profanity.is_bad_name(self._getname(full=True)):
            self._sessionplayer.setname('Censored', 'Censored Name', real=True)
...
def _update_text(self) -> None:
    ...
    fin_color = bs.safecolor(self.get_color()) + (1,)
    if not self._inited:
        bs.animate_array(
            self._text_node,
            'color',
...
def enable():
    _lobby.Chooser.__init__ = new_init
