...
class Icon(bs.Actor):
    ...
    def __init__(
        ...
        *,
        ...
        clr = bs.safecolor(player.team.color, target_intensity=1.0)
        device = player.sessionplayer.inputdevice
        device.get_v1_account_name(full=True)
        self._extra_text = bs.newnode(
            'text',
            owner=self.node,
            attrs={
                'text': device.get_v1_account_name(full=True),
                'color': clr + (1.0,),
                'h_align': 'center',
                'v_align': 'center',
                'vr_depth': 2,
    ...
    def set_position_and_scale(
        ...
        self._extra_text.position = (position[0], position[1] + scale * 85.0)
        self._extra_text.scale = 0.8 * scale * self._name_scale
        if self._show_lives:
...
class EliminationGame(bs.TeamGameActivity[Player, Team]):
    ...
    def get_available_settings(
        ...
        settings = [
            ...
            bs.BoolSetting('Revive Eliminated Players', default=False),
            bs.IntChoiceSetting(
    ...
    def __init__(self, settings: dict):
        ...
        self._revive_eliminated = bool(
            settings.get('Revive Eliminated Players', True)
        )
        self._revive_eliminated_timer: bs.Timer | None = None
        self._revive_time = int(
            settings.get('Revival Delay (Not Epic Time)', 120)
        )
        self._revive_count = int(settings.get('Players to Revive', 2))
    ...
    def spawn_player(self, player: Player) -> bs.Actor:
        ...
        if self._equip_gloves:
            actor.default_boxing_gloves = True
            actor.equip_boxing_gloves()
        if self._equip_shield:
            actor.default_shields = True
            actor.equip_shields()
        ...
        bomb_count = (
            actor.bomb_count if self._bomb_count == 0 else self._bomb_count
        )
        actor.bomb_count = actor._bomb_count = bomb_count
        bomb_type = (
            actor.bomb_type if self._bomb_type == 'default' else self._bomb_type
        )
    ...
    def _update(self) -> None:
        ...
        # Start the eliminated-revival timer when only 2 players remain.
            ...
            len(self._get_living_players()) == 2
            and self._revive_eliminated
            and len(self.players) >= 4
        ):
            self._revive_eliminated = False
            bs.broadcastmessage(
                f'Be ready! ⚔️ {self._revive_count} random eliminated players may rejoin in {self._revive_time / 60:g} minute(s) to finish the game.',
                color=(1, 0.7, 0.1),
            )
            self._revive_eliminated_timer = bs.BaseTimer(
                self._revive_time,
                bs.WeakCallStrict(self._revive_random_players),
            )
    ...
    def _revive_random_players(self) -> None:
        ...
        eliminated_players = [
            p for p in self.players if p.exists() and p.lives <= 0
        ]
        if not eliminated_players:
            bs.broadcastmessage(
                'No eliminated players available.', color=(0.7, 0.7, 0.7)
            )
            return

        # Pick up at least 1 random eliminated players
        revived = random.sample(
            eliminated_players, min(self._revive_count, len(eliminated_players))
        )
        for player in revived:
            ...
            bs.broadcastmessage(
                f'{player.getname(full=True)} rejoined the game!',
                color=(0, 1, 0),
            )
