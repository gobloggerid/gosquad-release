    ...
    def spawn_player(self, player: Player) -> bs.Actor:
        if player.team.id == 0:
            spaz = self.spawn_player_spaz(
                player, position=(-3.02237, 3.52416, -2.81289)
            )
            return spaz
        elif player.team.id == 1:
            spaz = self.spawn_player_spaz(
                player, position=(3.67858, 3.52384, -2.77144)
            )
            return spaz
