...
class BaseRaidGame(ba.TeamGameActivity[Player, Team]):
    ...
    def on_begin(self) -> None:
        ...
        death_material.add_actions(
                 ...
                 actions=(('modify_part_collision', 'collide', True),
                         ...
                         ('call', 'at_connect', ba.WeakCallPartial(self.handle_region, 'ground'))))
        box_1.add_actions(
                 conditions=('they_have_material', palyer),
                 actions=(('modify_part_collision', 'collide', True),
                         ('modify_part_collision', 'physical', False),
                         ('call', 'at_connect', ba.WeakCallPartial(self.handle_region, 'turret_1'))))
        box_2.add_actions(
                 conditions=('they_have_material', palyer),
                 actions=(('modify_part_collision', 'collide', True),
                         ('modify_part_collision', 'physical', False),
                         ('call', 'at_connect', ba.WeakCallPartial(self.handle_region, 'turret_2'))))
        ...
        if self._epic_mode:
            ...
            delay = 3.0
        ba.timer(delay, ba.WeakCallPartial(self.selection, first_run = True))
        teams = []
        for t in self.teams:
            teams.append(t)
        roles = ['raid', 'defend']
    ...
    def decrease(self) -> None:
        if int(self.time) != 0:
            ...
            self.knock_players()
            self.knocker = ba.Timer(0.8, ba.CallPartial(self.knock_players), repeat=True)
    ...
    def selection(self, first_run: bool = False) -> None:
        ...
        def scene_2():
            ...
            self.loop_timer = ba.Timer(1, ba.CallPartial(self.decrease), repeat=True)
        ...
        ba.timer(0.1, text)
        ba.timer(0.3, scene)
        ba.timer(1.0 if not first_run else 0.8, scene_2)
    ...
    def _standard_drop_powerups(self) -> None:
        for i in range(4 if self.ground else 8):
            ba.timer(i * 0.4, ba.WeakCallPartial(self._standard_drop_powerup, i))
