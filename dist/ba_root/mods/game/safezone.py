...
class SafeZoneGame(bs.TeamGameActivity[Player, Team]):
    ...
    def on_begin(self) -> None:
        ...
        bs.timer(3, babase.CallStrict(self.add_bot, 'left'))
        bs.timer(3, babase.CallStrict(self.add_bot, 'right'))
        if len(self.initialplayerinfos) > 4:
            bs.timer(5, babase.CallStrict(self.add_bot, 'right'))
            bs.timer(5, babase.CallStrict(self.add_bot, 'left'))
    ...
    def spawn_zone(self):
        ...
        bs.animate_array(
            ...
            {
                0: [0],
                0.3: [self.get_players_count() * 0.85],
                0.35: [self.get_players_count() * 0.8],
            },
        )
        bs.animate_array(
            self.zone_limit,
            ...
            {
                0: [0],
                0.3: [self.get_players_count() * 1.2],
                0.35: [self.get_players_count() * 0.95],
            },
        )
    ...
    def move_zone(self):
        ...
        new_pos = (x, 0.05, y)
        bs.animate_array(
            self.zone, 'position', 3, {0: self.zone.position, 8: new_pos}
        )
        bs.animate_array(
            self.zone_limit,
            'position',
            3,
            {0: self.zone_limit.position, 8: new_pos},
        )
    ...
    def start_timer(self):
        ...
        self._timer_x = bs.Timer(1.0, bs.WeakCallStrict(self.tick), repeat=True)
    ...
    def check_players(self):
        if self._time_remaining <= 0:
            ...
            bs.animate_array(
                ...
                1,
                {
                    0: [self.last_players_count * 0.8],
                    1.4: [self.last_players_count * 0.8],
                    1.5: [0],
                },
            )
            bs.animate_array(
                ...
                1,
                {
                    0: [self.last_players_count * 0.95],
                    1.45: [self.last_players_count * 0.95],
                    1.5: [0],
                },
            )
    ...
    def spawn_player(self, player: Player) -> bs.Actor:
        ...
        if not self._solo_mode:
            bs.timer(0.3, babase.CallStrict(self._print_lives, player))
    ...
    def _on_spaz_bot_died(self, die_msg):
        bs.timer(
            1, babase.CallStrict(self.add_bot, die_msg.spazbot.node.position)
        )
    ...
    def _get_bot_speed(self, bot_type):
        if bot_type == stdbot.BomberBotProStaticHyper:
            ...
            raise Exception(
                'Invalid bot type to _getBotSpeed(): ' + str(bot_type)
            )
