...
class MeteorShowerGame(bs.TeamGameActivity[Player, Team]):
    ...
    available_settings = [
        ...
        bs.BoolSetting('Default Bomb', default=False),
    ]
    ...
    def __init__(self, settings: dict):
        ...
        self._default_bomb = bool(settings.get('Default Bomb', False))
    ...
    def _drop_bomb_cluster(self) -> None:
        ...
        for _i in range(random.randrange(1, 3)):
            ...
            if self._default_bomb:
                bs.timer(delay, bs.CallStrict(self._drop_bomb, pos, vel))
            else:
                bs.timer(delay, bs.CallStrict(self._drop_bomb_mod, pos, vel))
            delay += 0.1
    ...
    def _drop_bomb_mod(
        ...
        autoaim = (
            True
            if random.choices([True, False], weights=[0.05, 0.95])[0]
            else False
        )
        density = 0.5 if autoaim else 1.0
        gravity_scale = 0.5 if autoaim else 1.0
        bomb = Bomb(
            position=position,
            velocity=velocity,
            bomb_type=bomb_type,
