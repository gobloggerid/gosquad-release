...
class KeepAwayGame(bs.TeamGameActivity[Player, Team]):
    ...
    def _is_holding_flag(self, spaz) -> bool:
        """Check if a given spaz (actor or puppet) is holding a flag."""
        try:
            if spaz and spaz.node and spaz.node.hold_node:
                return spaz.node.hold_node.getnodetype() == 'flag'
        except Exception:
            logging.exception('Error checking hold flag on spaz.')
        return False
    ...
    def _update_flag_state(self) -> None:
        ...
        for player in self.players:
            ...
            try:
                ...
                # check main actor
                if self._is_holding_flag(player.actor):
                    holdingflag = True
                # also check puppetspaz if present
                elif getattr(player.actor, '_puppetspaz', None):
                    if self._is_holding_flag(player.actor._puppetspaz):
                        holdingflag = True
...
def apply() -> None:
    """Apply this overlay onto bascenev1lib.game.keepaway."""
    import importlib
    import sys

    orig_module = importlib.import_module('bascenev1lib.game.keepaway')
    orig_game = orig_module.KeepAwayGame
    ...
    # Rebind simple module-level aliases from patched spaz module object.
    for mod in list(sys.modules.values()):
        if mod is None:
            print('game is none')
            continue
        if getattr(mod, 'KeepAwayGame', None) is orig_game:
            mod.KeepAwayGame = KeepAwayGame
