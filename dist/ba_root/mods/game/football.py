...
class FootballTeamGame(bs.TeamGameActivity[Player, Team]):
    ...
    def __init__(self, settings: dict):
        ...
        self.default_music = bs.MusicType.FOOTBALL
...
def apply() -> None:
    """Apply this overlay onto bascenev1lib.game.football."""
    import importlib
    import sys

    orig_module = importlib.import_module('bascenev1lib.game.football')
    orig_game = orig_module.FootballTeamGame
    orig_coopgame = orig_module.FootballCoopGame
    ...
    for mod in list(sys.modules.values()):
        if mod is None:
            print('game is none')
            continue
        if getattr(mod, 'FootballTeamGame', None) is orig_game:
            mod.FootballTeamGame = FootballTeamGame
        if getattr(mod, 'FootballCoopGame', None) is orig_coopgame:
            mod.FootballCoopGame = FootballCoopGame
