...
class Player(bs.Player['Team']):
    ...
    def __init__(self) -> None:
        ...
        self.invisible_timer: bs.Timer | None = None
...
class InvisibleOneGame(bs.TeamGameActivity[Player, Team]):
    """
    ...
    available_settings = [
        ...
        bs.BoolSetting('Chosen One Gets Gloves', default=False),
    ...
    def __init__(self, settings: dict):
        ...
        self._invisible_time = int(settings['Invisible Time'])
    ...
    def on_begin(self) -> None:
        ...
        self.globalsnode.tint = (0.5, 0.7, 1.0)
    ...
    def _tick(self) -> None:
        ...
        if player is not None:
            ...
            if not player.is_alive():
                ...
                # Show the count over their head if they're visible.
                if not player.invisible and scoring_team.time_remaining > 0:
    ...
    def _turn_invisible(self, player: Player) -> None:
        from goactor.extraspaz import turn_invisible

        if player.actor:
            turn_invisible(player.actor)
            player.invisible = True
            player.actor.invisible = True
            self.invisible_timer = bs.Timer(
                self._invisible_time,
                bs.WeakCallStrict(self._turn_visible, player),
            )

    def _turn_visible(self, player: Player) -> None:
        player.invisible = False
        if player.actor:
            player.actor.invisible = False
            player.actor.default_appearance()
