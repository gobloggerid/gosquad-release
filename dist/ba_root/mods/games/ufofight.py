...
class UFO(bs.Actor):
    ...
    def __init__(self, hitpoints: int = 5000):
        ...
        self._scoreboard()
        self._update()
        self.drop_bomb_timer = bs.Timer(1.5, bs.CallStrict(self._drop_bomb),
                                        repeat=True)
    ...
    def _levitate(self):
        ...
        if node.exists():
            ...
            if not self.frozen:
                for i in range(7):
                    bs.timer(0.05 + i / 20, bs.CallPartial(raise_player, p))
        ...
        if isinstance(msg, bs.HitMessage):
            ...
            if self.node:
                ...
                for i in range(6):
                    ...
                    bs.timer(0 + i, bs.CallPartial(ded_explode, i))
            ...
            if not self.frozen:
                ...
                def unfrozen():
                    ...
                    self.drop_bomb_timer = bs.Timer(1.5,
                                                    bs.CallStrict(self._drop_bomb),
                                                    repeat=True)

                    self.drop_bots_timer = bs.Timer(15.0,
                                                    bs.CallStrict(self._drop_bots),
                                                    repeat=True)
    ...
    def _update(self) -> None:
        ...
        for player in bs.getactivity().players:
            ...
            try:
                ...
                actor = player.actor
                if player.is_alive() and actor is not None and actor.node:
                    player_pts.append(
                        (
                            bs.Vec3(actor.node.position),
                            bs.Vec3(actor.node.velocity),
                        )
                    )
            except Exception:
