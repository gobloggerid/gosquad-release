...
class PlayerSpaz_Zom(PlayerSpaz):
    def handlemessage(self, m: Any) -> Any:
        if isinstance(m, bs.HitMessage):
            if not self.node:
                return
            if m._source_player is not None:
                try:
...
class zBotSet(SpazBotSet):
    def start_moving(self) -> None:
        ...
        self._bot_update_timer = bs.Timer(
            0.05, bs.WeakCallStrict(self.zUpdate), repeat=True
        )
    ...
    def zUpdate(self) -> None:
        ...
        for player in bs.getactivity().players:
            ...
            try:
                ...
                babase.applog.exception('Error on bot-set _update.')
...
class ZombieHorde(bs.TeamGameActivity[Player, Team]):
    ...
    def on_begin(self) -> None:
        ...
        BrawlerBot.color = 0.4, 0.1, 0.05
        BrawlerBot.highlight = 0.2, 0.4, 0.3
        BrawlerBot.character = 'Kronk2'
    ...
    def _update(self) -> None:
        ...
        if len(teamsRemain) < 2:
            if len(teamsRemain) == 1:
                ...
                if teamsRemain[0].score < max(theScores) or (
    ...
    def end_game(self) -> None:
        if self.has_ended():
            return
        BrawlerBot.color = 0.6, 0.6, 0.6
        BrawlerBot.highlight = 0.6, 0.6, 0.6
        BrawlerBot.character = 'Kronk'
