...
class SimonSays(bs.TeamGameActivity[Player, Team]):
    ...
    def __init__(self, settings: dict):
        ...
        self.options = ["red", "green", "blue", "yellow", "teal", "purple", "gray", "orange",
                        "white", "top", "bottom", "middle row", "left", "right", "center column",
                        "outside"]
    ...
    def on_begin(self) -> None:
        super().on_begin()
        bs.animate_array(self.n1, 'size', 1, {0: [0.0], 0.2: [self._r1 * 2.0]})
        bs.animate_array(self.n2, 'size', 1, {0: [0.0], 0.2: [self._r1 * 2.0]})
        bs.animate_array(self.n3, 'size', 1, {0: [0.0], 0.2: [self._r1 * 2.0]})
        bs.animate_array(self.n4, 'size', 1, {0: [0.0], 0.2: [self._r1 * 2.0]})
        bs.animate_array(self.n5, 'size', 1, {0: [0.0], 0.2: [self._r1 * 2.0]})
        bs.animate_array(self.n6, 'size', 1, {0: [0.0], 0.2: [self._r1 * 2.0]})
        bs.animate_array(self.n7, 'size', 1, {0: [0.0], 0.2: [self._r1 * 2.0]})
        bs.animate_array(self.n8, 'size', 1, {0: [0.0], 0.2: [self._r1 * 2.0]})
        bs.animate_array(self.n9, 'size', 1, {0: [0.0], 0.2: [self._r1 * 2.0]})
        for team in self.teams:
            team.score = 0
        for player in self.players:
            player.score = 0
        # check for immediate end if there's only 1 player
        if len(self.players) == 1:
            bs.timer(4000 / 1000, bs.CallStrict(self.check_end))
    ...
    def call_round(self) -> None:
        ...
        false_prefix = random.choices(['Simon say r', 'Simon said r', 'Simon r',
                                       'Simons says r', 'Simons r', 'R'],
                                      weights=[35, 45, 45, 39, 49, 100])[0]
        if self.numa < 9:
            if not self.simon:
    ...
    def in_circle(self, pos: Sequence[float]) -> list[str]:
        circles: list[str] = []
        x = pos[0]
        z = pos[2]
        if (x + 4) ** 2 + (z + 6) ** 2 < 4:
    ...
    def end_game(self) -> None:
        self.ended = True
        # Award surviving players a score higher than any eliminated player.
        for player in self.players:
            if player.is_alive() and not player.dead:
                player.team.score = self.round_num + 1
    ...
    def check_end(self) -> None:
        ...
        if isinstance(self.session, bs.CoopSession):
            if i <= 0:
                bs.timer(0.6, bs.CallStrict(self.end_game))
        else:
            if i <= 1:
                bs.timer(0.6, bs.CallStrict(self.end_game))
