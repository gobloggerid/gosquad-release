...
class Rocket(bs.Actor):
    ...
    def __init__(
        ...
        self._life_timer = bs.Timer(
            5, bs.WeakCallStrict(self.handlemessage, bs.DieMessage())
        )

        self._emit_timer = bs.Timer(
            0.001, bs.WeakCallStrict(self.emit), repeat=True
        )
...
class RailBullet(bs.Actor):
    ...
    def __init__(
        ...
        bs.animate(
            self.node,
            'radius',
            {0: MIN_SPHERE_SIZE, 0.1: 0.5, 0.5: MIN_SPHERE_SIZE},
        )

        self.source_player = source_player
        self.owner = owner
        self._life_timer = bs.Timer(
            0.5, bs.WeakCallStrict(self.handlemessage, bs.DieMessage())
        )
...
class QuakeGame(bs.TeamGameActivity[Player, Team]):
    ...
    def __init__(self, settings) -> None:
        ...
        self._shield_dropper: bs.Timer | None = None
    ...
    def on_begin(self) -> None:
        ...
        self._shield_dropper = bs.Timer(
            8, bs.WeakCallStrict(self.drop_shield), repeat=True
        )
