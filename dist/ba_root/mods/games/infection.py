...
class InfectionGame(bs.TeamGameActivity[Player, Team]):
    ...
    def on_begin(self) -> None:
        ...
        self.mine_count = 0
        bs.timer(
            self._update_rate, bs.WeakCallStrict(self._mine_update), repeat=True
        )
        bs.timer(
            self._max_size_increases * 1.0,
            bs.WeakCallStrict(self._max_size_update),
            repeat=True,
        )
        bs.timer(
            self._extra_mines * 1.0,
            bs.WeakCallStrict(self._max_mine_update),
            repeat=True,
        )
    ...
    def _mine_update(self) -> None:
        ...
        if self.mine_count < self._max_mines:
            ...
            bs.timer(0.95, babase.CallStrict(self._make_mine, pos))
