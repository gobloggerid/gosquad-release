...
class Ball(bs.Actor):
    def __init__(self, position: Sequence[float] = (0.0, 1.0, 0.0)):
        ...
        assert isinstance(activity, BasketBallGame)
        ...
        bs.animate(
            self.node,
            'mesh_scale',
            {0: MIN_SPHERE_SIZE, 0.2: scale * 1.3, 0.26: scale},
        )
...
class BasketBallGame(bs.TeamGameActivity[Player, Team]):
    """Basket Ball game."""
    ...
    def get_instance_description(self) -> str | Sequence:
        return 'Score ${ARG1} baskets to win!', self._score_to_win
