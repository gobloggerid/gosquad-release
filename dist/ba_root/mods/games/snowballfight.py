# Porting to api 9 made easier by baport.(https://github.com/bombsquad-community/baport)
...
class Snowball(bs.Actor):
    def __init__(
        ...
        self.scale = max(MIN_SPHERE_SIZE, float(bomb_scale))
        ...
        self.radius = max(MIN_SPHERE_SIZE, self.scale * 0.1)
        if self.scale <= 1.0:
            shadow_size = 0.6
        elif self.scale <= 2.0:
            shadow_size = 0.4
        ...
        bs.animate(
            self.node,
            'mesh_scale',
            {0: MIN_SPHERE_SIZE, 0.2: 1.3 * self.scale, 0.26: self.scale},
        )
        bs.animate(
            self.light,
            'radius',
            {0: MIN_SPHERE_SIZE, 0.2: 1.3 * self.radius, 0.26: self.radius},
        )
        if self.snowball_melt:
            bs.timer(1.5, bs.WeakCallStrict(self._disappear))
    ...
    def hit(self) -> None:
        ...
        if self.snowball_explode:
            ...
            self.do_explode()
            bs.timer(
                0.001, bs.WeakCallStrict(self.handlemessage, bs.DieMessage())
            )
        else:
            self.do_hit()
    ...
    def do_hit(self) -> None:
        ...
        if babase.Vec3(*v).length() > 5.0:
            ...
            if node is not None and node and node not in self._hit_nodes:
                ...
                node.handlemessage(
                    bs.HitMessage(
                        pos=t,
                        velocity=v,
                        magnitude=babase.Vec3(*v).length() * 0.8,
                        velocity_magnitude=babase.Vec3(*v).length() * 0.8,
                        radius=0,
                        srcnode=self.node,
        ...
        if not self.snowball_bounce:
            bs.timer(0.05, bs.WeakCallStrict(self.do_bounce))
    ...
    def bounce(self) -> None:
        ...
        if not self.snowball_bounce:
            ...
            bs.timer(0.01, bs.WeakCallStrict(self.calc_bounce, vel))
        else:
            return
    ...
    def do_effect(self) -> None:
        ...
        bs.animate(
            self.light,
            'radius',
            {0.0: lr * 1.0, 0.02: lr * 0.5, 0.05: MIN_SPHERE_SIZE},
        )
        bs.timer(0.08, bs.WeakCallStrict(self.handlemessage, bs.DieMessage()))
    ...
    def _disappear(self) -> None:
        ...
        if self.node:
            ...
            bs.animate(
                self.light,
                'radius',
                {0.0: lr * 1.0, 0.3: lr * 0.5, 0.5: MIN_SPHERE_SIZE},
            )
            bs.timer(
                0.55, bs.WeakCallStrict(self.handlemessage, bs.DieMessage())
            )
